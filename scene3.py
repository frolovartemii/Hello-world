...
def scene3_frame_1(second_hall, body_2, products_warehouse, arsenal, rise, stair, race):
    ...
    return choise
def scene3_frame_2():
    print(" В середине зала лежит мёртвый эльф. По бокам зала распологаются две двери, одна с надписью *Хранилище еды*, другая *место хранения священного огня*.В конце зала находится выход в коридор.")
    second_hall = True
    return second_hall
...
def scene3_frame_4(e, f, sunpriest_key, Key, race, poach, powder_case, powder_poach, sugar, money, powder_case_BOOM, hands):
    ...
    if e == 1:
        while c != "1":
            print(" 1 - Выйти из места хранилища священного огня.")
            if f == 0:
            ...
            if c == "2" and f == 0:
                ...
                f += 1
            if c == "3" and f == 1 and powder_case == False and powder_poach == False and poach == True:
                if sugar == True:
...
def scene3_frame_5(g, flour, powder_poach, money, sugar, poach):
    ...
    while b != 1:
        ...
        if b == "2" and flour == False:
            ...
            flour = True
        if b == "3" and sugar == False and poach == True:
            if money == True:
                money = False
