        ...
        if backtracking_2 == False:
            ...
            while stair  == False:
                ...
                if choise == "1" and second_hall == False:
                    ...
                    a, b, c, d, powder_line, powder_poach, main_exit= scene2.scene2_frame_6(a, b, c, d, race, powder_case, powder_poach, powder_line, Margancovka, sugar, main_exit, hands)
                    if main_exit:
                        break
        ...
        if backtracking_4 == False:
            while main_exit == False:
                if intro4 == 0:
                    intro4 += 1
                    scene4.intro()
                choise = scene4.scene4_frame_1(third_hall, rise, body_3)
                if choise == "1" and third_hall == False:
                    third_hall = scene4.scene4_frame_2()
                    ...
                    body_3, Head, Margancovka, sunpriest_key = scene4.scene4_frame_4(body_3, name, race, Head, Margancovka, sunpriest_key)
                elif choise == "3" and body_3 == True:
                    sugar, sugar_poach = scene4.scene4_frame_5(name, sugar, sugar_poach, main_exit, powder_case_BOOM, Margancovka)
                elif choise == "4" and third_hall == True:
                    powder_case_BOOM, i, powder_case = scene4.scene4_frame_6(i, powder_case, powder_case_BOOM)
                elif choise == "5" and money == True and third_hall == True:
                    main_exit = scene4.scene4_frame_7(main_exit, sugar_poach)
                elif choise == "6" and money == True and third_hall == True:
                    main_exit = scene4.scene_frame_8(sugar_hole, main_exit, elf_claw, flour)
                else:
                    print("Неверно введено, попробуй ещё раз.")
            if main_exit:
                break
    print("Поздравляю, вы выбрались из первого подземелья, из из моей демо-версии игры. Если вы хотите увидить продолжение, то можете поддержать автора монетой скинув любую сумму денег на этот номер: 8-927-035-02-36. Ваша поддержка очень важна мне!")
else:
