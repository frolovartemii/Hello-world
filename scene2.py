...
def scene2_frame_2(race, name, first_hall):
    ...
    if race == "эльф":
        print(f"({name})- Понятно что это за место, склеп служителей солнца. Всегда их ритуал погребения казался мне странным, по моему мнению, семье покойника не должно хотеться есть за столом, на котором лежит труп, после чего ещё и учавствовать в сжигании его тела. Хотя каких фанатиков только нет в мире.")
    first_hall = True
    return first_hall
...
def scene2_frame_6(a, b, c, d, race, powder_case, powder_poach, powder_line, Margancovka, sugar, main_exit, hands, Stels_exit):
    answer = ""
    answer_2 = ""
    if a == 0:
        if race == "человек":
    ...
    while answer != "3":
        ...
        if answer == "1" and powder_line == False:
            ...
            print(" Не спорь с рассказчиком. Выбирай то что дают.")
    return a, b, c, d, powder_line, powder_poach, main_exit, Stels_exit, Margancovka
